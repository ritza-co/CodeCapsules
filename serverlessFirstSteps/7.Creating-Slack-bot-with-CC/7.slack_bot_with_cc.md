...
# Talking to your code, using a Slackbot with Node.js
...
One of the stand out features is the rich API Slack exposes, to allow developers to integrate with it. In this tutorial, we'll use the Slack API to create a Slackbot that we can talk to our apps running on Code Capsules, to monitor their status and see if they are up and running. 
...
We'll also need a place to host our app. 
...
To start, clone your new GitHub repo onto your local computer.
...
- [Express](expressjs.com). This acts as our web server and http request router. We'll use this to route requests from Slack to the correct logic. 
- [body-parser](body-parser). This interprets and parses payload data from http requests. We'll need this to parse the url-encoded data Slack sends with a request to us. 
- [superagent](superagent). This package allows us to make outgoing http requests. We'll need this to send a message to Slack. 
...
Let's add the boiler-plate code to startup a new Express server: 
...
Then it replies to the request in the format specified by Slack in their docs. We use the line `vtext: JSON.stringify(stats, null, '\t')` to turn our info and stats object into a nicely formatted text string, in the style of a JSON object. 
