...
Deploying a web application to a production environment is complex. Traditionally one must set up a server and micromanage the tasks associated with hosting a server: choosing the servers' operating system, editing its configurations, and monitoring the server to ensure it is running.
