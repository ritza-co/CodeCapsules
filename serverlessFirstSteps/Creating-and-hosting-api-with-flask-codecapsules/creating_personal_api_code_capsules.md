...
Before creating an API, we need to understand what one is. An API, or Application Programming Interface, is a tool that enables developers to interact with other people's (and companies) data. For example, you navigate to a weather website and see your locations current temperature displayed on the homepage. How did they display your location's temperature?

Without a doubt, they used an API. APIs are hosted on a server and act as an access point between you (the user) and the individuals or companies data. Developers access this data via APIs and create applications with the data.
...
First we need to set up a virtual Python environment, using Virtualenv. Virtualenv provides a "blank" Python install - an install without any 3rd-party libraries or packages. Let's set up the environment.

1. Open your terminal and create an empty folder - this folder will contain your API.
...
- [Requests](https://pypi.org/project/requests/) is a Python library will help us interact with APIs.

From your virtual environment, install these libraries using `pip3 install flask gunicorn requests`. 
...
To use these APIs, we need to register for each them to recieve a unique API key. An API key authenticates a user and ensures they are allowed to use the API. Let's create an account and get API keys for OpenExchangeRates and WeatherStack. 
...
We've obtained our API keys - now we can test them. Open a terminal in the folder we created earlier and re-activate the virtual environment. 
...
We use the requests module to "get" the data from the API, accessed through the EXCHANGE_URL variable. Near the end of the URL, notice the `/api/latest.json`. 

The `latest` signifies an API _endpoint_. OpenExchangeRates has many endpoints. Each endpoint provides different data. One could request data from the `historical` endpoint (`https://openexchangerates.org/api/historical/`) to access past exchange rates.
...
Now we've included an `EXCHANGE_PARAMS` variable. Providing an API parameter determines the received. In this case, we supplied the OpenExchangeRates parameter `symbols` with our three currencies.

Parameters vary depending on the API endpoint. A  list of parameters for the `latest` endpoint is [here](https://docs.openexchangerates.org/docs/latest-json). 


Now that we've obtained the exchange rates, let's retrieve the temperature for a city. Modify our code by adding the following code black below the `print(exchange_data.json())` line. This time, replace 'YOUR-API-KEY-HERE' with your WeatherStack API key.
...
We have our data. Let's use it to create our API using Flask.
...
Now we can make our API. When we request data from our API, we want to return the temperature from a city, and the exchange rates of three currencies. We've already written code to retrieve this data - now we must "combine" it and give it a location on our Flask application. 
...
The API is completed. Now to host it.
...
Create a new file named `Procfile` within the same directory. Open it and enter the following code:
